...
rr5data = xlsread('RR05 Data retrieval 180123.xlsx'); % Change number to set the day
...
figure; plot(auROCu_inc, '.-');  title('auROC unrewarded/rewarded head entries');
hold on; plot(auROCr_inc, 'r.-');
...
subplot(2,2,4); plot(auROCr_inc, 'go-'); ylim([0.1,0.9]); 
...
yticklabels({'lever','dipper','rewarded','unrewarded'}); title('Correlation Coefficients');
...
subplot(1,4,1); plot(auROCd(indexr), 'ro');  title('dipper presentations'); ylim([0.1,0.9]);
subplot(1,4,3); plot(auROCr(indexr), 'o'); title('rewarded head entries'); ylim([0.1,0.9]);
...
figure; subplot(1,4,2); plot(auROC(index), 'o'); title('lever presses'); ylim([0.1,0.9]);
subplot(1,4,1); plot(auROCd(index), 'ro');  title('dipper presentations'); ylim([0.1,0.9]);
...
subplot(1,4,1); plot(auROCd(indexu), 'ro');  title('dipper presentations'); ylim([0.1,0.9]);
subplot(1,4,3); plot(auROCr(indexu), 'o'); title('rewarded head entries'); ylim([0.1,0.9]);
subplot(1,4,4); plot(auROCu(indexu), 'o'); title('unrewarded head entries'); ylim([0.1,0.9]);
%set(gcf,'position',widegraph)
saveas(gcf,'auROC_indexed_to_reward.png');
...
%% average of all zones before & after each event
...
r_before1z = [];
r_before_navg = mean(r_before,2);
for i = 1:(size(r_before,1)/zs)
    r_before1z(i,:) = r_before_navg(i*zs-(zs-1):i*zs,1);
end
r_before_avg = mean(r_before1z,1);
r_after_navg = mean(r_after,2);

r_after1z = [];
for i = 1:(size(r_after,1)/zs)
    r_after1z(i,:) = r_after_navg(i*zs-(zs-1):i*zs,1);
end
r_after_avg = mean(r_after1z,1);
subplot(2,2,2); 
plot([r_before_avg';r_after_avg']); title('average zones before & after rewarded head entries');
ylim([0,0.3]);
hold on; plot([zs+0.5,zs+0.5],[0,0.3], '--');
...
wjet = [1,1,1;0.2,0.45,0.50;0.7,0.7,0.70;0.929,0.84,0.1250];
figure; colormap(wjet);

% lever press
image10p = 2*(n_image) + double(sum(neuron_image(:,:,index(size(index,2)-10:size(index,2))),3)>0)-double(sum(neuron_image(:,:,index(1:10)),3)>0);
subplot(2,2,1); imagesc(image10p); title('Top 10 lever press neurons');

% dipper presentations
image10d = 2*(n_image) + double(sum(neuron_image(:,:,indexd(size(indexd,2)-10:size(indexd,2))),3)>0)-double(sum(neuron_image(:,:,index(1:10)),3)>0);
subplot(2,2,2); imagesc(image10d); title('Top 10 dipper presentation neurons');

% rewarded entries
image10r = 2*(n_image) + double(sum(neuron_image(:,:,indexr(size(indexr,2)-10:size(indexr,2))),3)>0)-double(sum(neuron_image(:,:,index(1:10)),3)>0);
subplot(2,2,3); imagesc(image10r); title('Top 10 rewarded entry neurons');

% unrewarded entries
image10u = 2*(n_image) + double(sum(neuron_image(:,:,indexu(size(indexu,2)-10:size(indexu,2))),3)>0)-double(sum(neuron_image(:,:,index(1:10)),3)>0);
subplot(2,2,4); imagesc(image10u); title('Top 10 unrewarded entry neurons');
...
% subplot(2,2,3); plot(auROCd_inc(find(grpd==1)),'o'); hold on; 
% plot(auROCd_inc(find(grpd==2)),'ro'); hold on;
% plot(auROCd_inc(find(grpd==3)),'ko');
...
ngrps=3;
[grpu,idxu]=kmeans(auROCu_inc',ngrps,'replicates',200);
% figure; plot(auROC_inc(find(grpp==1)),'o'); hold on; 
% plot(auROC_inc(find(grpp==2)),'ro'); hold on;
% plot(auROC_inc(find(grpp==3)),'ko');

idxu = [[1:ngrps]',idxu];
idxu = sortrows(idxu,2);

grpu_tmp=grpu;
grpu_tmp(find(grpu==1))=idxu(1,1);
grpu_tmp(find(grpu==2))=idxu(2,1);
grpu_tmp(find(grpu==3))=idxu(3,1);
grpu=grpu_tmp;
grpu_tmp(1:10);
%
grp1u = sum(neuron_image(:,:,find(grpu==1)),3)>0;
grp2u = sum(neuron_image(:,:,find(grpu==2)),3)>0;
grp3u = sum(neuron_image(:,:,find(grpu==3)),3)>0;
imager = grp1u + 2*grp2u + 3*grp3u;
